...
- **禁止** import `trading_backtest`、`shioaji`、任何 app 層（theman 等）
- 同一 `TradingEngine` 狀態機同時服務 live 與 backtest
- Protocol 演進由 trading-engine 帶動（breaking 會 major bump）

## 3. Strategy Protocol 實作現況（trading-engine 0.2.0）
...
詳細預設值與校準流程請參考 consuming app 的 config（theman 等）與本 package 歷史 UAT 文件。**本 package 不硬塞預設值**，全部來自 RuntimeConfig。
...
- Telegram、報表、儲存、參數 sweep 執行器（theman / app 層）
- 多商品、scale-in、部分出場（kernel position model 限制）

## 9. 依賴與版本策略

```toml
dependencies = ["trading-engine>=0.2.0,<1.0"]
```
...
## 11. 從 theman 遷移對照（歷史，給舊使用者）

| theman 原路徑                  | 目標（本 package）             |
|--------------------------------|--------------------------------|
| `src/strategy/vwap_momentum.py`| `src/strategy_vwap_momentum/strategy.py` |
| `src/strategy/trend.py`        | 同上（完整保留 + 大量文件強化） |
| `src/strategy/params.py`       | `params.py`（StrategyParams + sweep helpers） |
| `src/strategy/base.py`         | **移除**，改繼承 trading_engine `BaseStrategy` |
| indicators                       | 多數已不需；必要時用 trading_engine.indicators |

theman 現在只做薄層 re-export 與 app 專屬 wiring。
...
See also: [README.md](README.md)、trading-engine `docs/STRATEGY.md`、`core/strategy.py`、`SPEC.md`。
